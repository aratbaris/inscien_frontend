...
export default function RegulatoryWatchPage() {
  ...
  const loadBrief = useCallback(async (day: string) => {
    ...
    setError(null);
    try {
      const res = await fetchAgentBrief("regulatory", day);
      setData(res);
    } catch (e: unknown) {
  ...
  return (
    <div className={styles.page}>
      <AgentHeader
        domain="Regulation"
        title="Financial Regulation Monitor"
        description="Tracks enforcement actions, rulemaking, and supervisory changes from SEC, CFTC, FCA, ESMA, FDIC, OCC, and other financial regulators that affect capital markets, banking, and payments."
        meta={[
      ...
      <main className={styles.content}>
        ...
        {!loading && !error && data && data.items.length === 0 && (
          <EmptyState
            ...
            hint="This can happen on days with no material financial regulatory developments."
          />
        )}
